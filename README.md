# 👋 Hi, I'm Bohdan Semybratov

🚀 **Junior iOS Developer**

Swift • SwiftUI • Mobile SDK Integrations • App Store Deployment

📍 Trnava, Slovakia  
📧 bohdan2003myr@gmail.com  

---

## 👨‍💻 About Me

Junior iOS Developer with experience in building mobile applications using **Swift and SwiftUI**.

I have practical experience integrating **mobile SDKs**, working with **App Store deployment**, and developing backend services using **Cloudflare Workers**.

Previously worked with **Unity (C#)** on mobile applications and game systems.

Currently focused on improving my **native iOS development skills** and building production-ready mobile apps.

## 🛠 Tech Stack

| Category | Technologies |
|--------|-------------|
| 📱 **iOS Development** | ![Swift](https://img.shields.io/badge/Swift-F54A2A?style=flat&logo=swift&logoColor=white) ![SwiftUI](https://img.shields.io/badge/SwiftUI-0A84FF?style=flat&logo=swift&logoColor=white) ![MVVM](https://img.shields.io/badge/MVVM-Architecture-blue?style=flat) ![REST API](https://img.shields.io/badge/REST_API-Integration-green?style=flat) ![JSON](https://img.shields.io/badge/JSON-Parsing-orange?style=flat) ![UserDefaults](https://img.shields.io/badge/UserDefaults-iOS-lightgrey?style=flat) |
| 🔌 **Mobile SDKs** | ![AppsFlyer](https://img.shields.io/badge/AppsFlyer-SDK-purple?style=flat) ![OneSignal](https://img.shields.io/badge/OneSignal-Push-red?style=flat) ![Firebase](https://img.shields.io/badge/Firebase-Analytics-yellow?style=flat&logo=firebase&logoColor=black) ![Unity Ads](https://img.shields.io/badge/Unity_Ads-Monetization-black?style=flat) |
| ☁ **Backend** | ![Cloudflare](https://img.shields.io/badge/Cloudflare-Workers-orange?style=flat&logo=cloudflare&logoColor=white) ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white) ![REST API](https://img.shields.io/badge/REST_API-Backend-green?style=flat) |
| 🎮 **Game Development** | ![Unity](https://img.shields.io/badge/Unity-Game_Engine-black?style=flat&logo=unity&logoColor=white) ![C#](https://img.shields.io/badge/C%23-Programming-purple?style=flat&logo=csharp&logoColor=white) ![Zenject](https://img.shields.io/badge/Zenject-DI_Framework-blue?style=flat) |
| 🌐 **Web Development** | ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white) |

## ⚡ Key Experience

### iOS Developer
**Confidential Company (NDA)** — Remote  
05/2025 – 03/2026

- Development of iOS applications using **Swift and SwiftUI**
- Integration of mobile SDKs (AppsFlyer, OneSignal)
- App deployment and release management in **App Store Connect**
- Handling App Store review issues
- Configuration of **AWS remote macOS environments** for iOS builds

---

### Unity Developer
**Confidential Company (NDA)** — Remote  
04/2024 – 05/2025

- Development of mobile applications and games using **Unity (C#)**
- Integration of third-party SDKs and analytics
- Gameplay mechanics and UI systems implementation
- Optimization and debugging of mobile builds

---

### Web Designer / Frontend Developer
**Confidential Company (NDA)** — Remote  
05/2023 – 04/2024

- Design and development of landing pages
- HTML/CSS layout implementation
- Responsive UI development

---

### Freelance Developer
Self-Employed  
09/2020 – Present

- Web development projects for small businesses
- Telegram bot development using Python
- Automation scripts development
- Landing page development (HTML, CSS, JavaScript)

---

## 📦 GitHub Projects

🚧 iOS apps and backend tools currently being prepared and published.

---

## 🎓 Education

**Junior Specialist – Computer Engineering**

Novograd-Volynskyi Applied Polytechnic College  
2019 – 2023

---

## 🌍 Languages

🇺🇦 Ukrainian — Native  
🇬🇧 English — Pre-Intermediate

---

## 🔗 Connect with me

LinkedIn  
https://linkedin.com/in/bohdan-semybratov

Telegram  
@b3njamin_1

---

⭐ Always open to interesting **iOS development opportunities**
